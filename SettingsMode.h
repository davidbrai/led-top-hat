...
public:
  ...
  void showSettings() { 
    ...
    PRINTX("incrementLeds: ", incrementLeds);
    PRINTX("numleds: ", NUM_LEDS);
    PRINTX("maxlevel: ", MAX_LEVEL);
    PRINTX("mmm: ", 138 / MAX_LEVEL);
    ...
    while (!_exitingSettings) { 
      ...
      for(uint8_t i = 1; i <= _userLevel; i++) {
        ...
        for (uint8_t x = 0; x < incrementLeds; ++x) { 
          ...
          leds[index] = CHSV(43*x, 255, i*incrementBrightness);
        }
      }
